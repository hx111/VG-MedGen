...
class MEncoder(nn.Module):
    def __init__(self, z_length, anatomy_out_channels):
        super(MEncoder, self).__init__()
        self.z_length = z_length
        input_channels = anatomy_out_channels + 3
        self.block1 = conv_bn_lrelu(input_channels, 16, 3, 2, 1)
...
class SDNet(nn.Module):
    ...
    def __init__(self, width, height, num_classes, ndf, z_length, norm, upsample, anatomy_out_channels, num_mask_channels):
        ...
        self.m_encoder = MEncoder(self.z_length, self.anatomy_out_channels)
    ...
    def forward(self, x, mask, script_type):
        ...
        return reco, z_out, mu_out_tilde, a_out_tilde, a_out, seg_pred, mu_out, logvar_out, mu_out, logvar_out